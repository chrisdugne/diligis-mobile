...
local MultipartFormData = require ("libs.oauth.multipartForm")
...
function networkRequest(url, customParams, consumer_key, token, consumer_secret, token_secret, method, listener)
    ...
    if method == "GET" then
	    network.request(post_data, method, listener, params)
    else
